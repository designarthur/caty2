<?php
// api/admin/download.php - Generates PDF for invoices and quotes
...
if ($type === 'invoice') {
    ...
    // Fetch quote data
    $stmt = $conn->prepare("SELECT q.*, u.first_name, u.last_name FROM quotes q JOIN users u ON q.user_id = u.id WHERE q.id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
